...
keyword = []
com = ["and", "that", "the", "for", "it", "it's", "was", "his", "who", "work", "used", "way", "also", "by", "can", "which", "as", "known", "then", "if",
       "between", "through", "another", "", "or", "my", "in", "from", "a", "any", "on", "combination", "to", "into", "is", "of", "It", "A", "each", "both"]
...
class Test(tk.Frame):
	def __init__(self):
		...
		self.entryA = Text(new, height=14, width=46, padx=5,
		                   pady=5, wrap=WORD, background='grey')
		self.entryA.grid(row=0, column=1, padx=13, pady=21, sticky=E)

		tk.Button(new, text='  Submit  ', default='active',
		          command=self.click_ok).grid(row=1, column=1, padx=13, pady=21)
	...
	def click_ok(self):
		...
		if text == "" or text == " ":
			messagebox.showinfo(
				"Interviewer: ", "I cant reciever you Please say something")
...
class App(tk.Frame):
	def __init__(self, master):
		...
		choices = {'E-Commerce', 'NLP', 'Cryptography',
		...
		tk.Button(self, text='  Submit  ', default='active', command=self.click_ok).grid(
			row=3, column=1, padx=13, pady=21, sticky=S)
...
class Report(tk.Frame):
	...
	def __init__(self):
		...
		new.button = tk.Button(new, text="Detailed Report",
		                       width=15, command=self.Det_report)
	...
	def nxt(self):
		...
		if re <= 5:
			...
			new.title("Intervew Hall")
			self.tk_setPalette(background='#d8bbaa')

			new.geometry("610x377+361+223")

			Quw = "Can you give an insight about the topic"
			tk.Label(new, text=Quw).grid(row=0, column=0, padx=13, pady=21, sticky=W)
			...
			tk.Button(new, text='  Submit  ', default='active',
			          command=self.ank).grid(row=1, column=1, padx=13, pady=21)
		else:
			new = Toplevel(self)
			...
			Quw = "You are good! explain some advancements of it"
			tk.Label(new, text=Quw).grid(row=0, column=0, padx=13, pady=21, sticky=W)
			...
			tk.Button(new, text='  Submit  ', default='active',
			          command=self.ranch).grid(row=1, column=1, padx=13, pady=21)

	def ank(self):
		messagebox.showinfo(
			"Interviewer: ", "more knowledge is needed,we should comeback to you")
...
class Det_Report(tk.Frame):
	...
	def __init__(self):
		...
		tk.Message(new, text=" Your Total Marks is = " + ans, font='Arial 10 underline',
		           justify='left', aspect=1500).grid(row=1, column=2, padx=2, pady=2, sticky=W)

		tk.Message(new, text=" The Similarity factor of the sentence is :            {:.2%}".format(
			fr/100), font='Arial 10 underline', justify='left', aspect=1500).grid(row=2, column=2, padx=2, pady=2, sticky=W)
		tk.Message(new, text=" The Grammar accuracy of the sentence is :              {:.2%}".format(
			gm), font='Arial 10 underline', justify='left', aspect=1500).grid(row=3, column=2, padx=2, pady=2, sticky=W)
		tk.Message(new, text=" The Total Keywords found in the sentence is :          {:.2%}".format(
			kt), font='Arial 10 underline', justify='left', aspect=10000).grid(row=4, column=2, padx=2, pady=2, sticky=W)

		tk.Message(new, text=" The Keyword order accuracy of the sentence is :        {:.2%}".format(
			cm), font='Arial 10 underline', justify='left', aspect=10000).grid(row=5, column=2, padx=2, pady=2, sticky=W)
		...
		tk.Message(new, text=" Some of the Sample Answers are: "+strans, font='System 14',
